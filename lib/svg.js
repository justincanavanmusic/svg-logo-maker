class Test {
    constructor() {
      
    }
}

class SVG extends Test {
  constructor() {
    super()
  }
    setText(text, textcolor) {
        this.text=text;
        this.textcolor=textcolor;
        if (text.length>3) {
            new Error("Text must not exceed 3 characters.")
        }
    }
    setShape(shape, shapecolor) {
        this.shape=shape;
        this.shapecolor=shapecolor;

    }

  render() {
    return `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg"></svg>`
}
   render2() {
    let text=this.text;
    let textcolor=this.textcolor;
        return `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg"><text x="116" y="135" font-size="60" text-anchor="middle" fill="${textcolor}">${text}</text></svg>`
    }
    render3() {
        let text=this.text;
        let textcolor=this.textcolor;
        let shapecolor=this.shapecolor;
    
        return  `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg"><<rect x="40px" y="40px" width="150px" height="150px" fill="dodgerblue" /><text x="116" y="135" font-size="60" text-anchor="middle" fill="${textcolor}">${text}</text></svg>`;
    }
}

module.exports=SVG




